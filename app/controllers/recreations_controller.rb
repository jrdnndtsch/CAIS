class RecreationsController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @recreation.save
        format.html { redirect_to admin_path, notice: 'Recreation was successfully created.' }
        format.json { render :show, status: :created, location: @recreation }
      else
  ...
  private
    ...
    def recreation_params
      params.require(:recreation).permit(:recreation_type, :activity, :recreation_type_id)
    end
end
